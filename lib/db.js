...
export async function listPaymentsSince(days = 30) {
  ...
  return sql`
    SELECT id, internal_order_id, bog_order_id, status, session_id,
           ...
           refund_status, refund_requested_amount, refunded_amount,
...
export async function deleteEventsByIds(ids) {
  ...
  // Booking history must survive an event delete (the archive tab reads
  // straight from sessions/bookings). Re-parent already-past sessions onto
  // the shared "movie no longer showing" placeholder before the CASCADE
  // wipes them along with the event; only future sessions (nothing to
  // archive yet) get deleted for real.
  let archived = await sql`
    SELECT id FROM events WHERE title = 'Фильм больше не показывается' LIMIT 1
  `;
  let archiveId = archived[0]?.id;
  if (!archiveId) {
  ...
  await sql`
    UPDATE sessions SET event_id = ${archiveId}
...
export async function getPublicFloorPlan(eventId) {
  const sql = db();
...
export async function getSessionDetail(sessionId) {
  ...
  const bookings = await sql`
    SELECT id, table_label, guest_name, guest_phone, guest_instagram,
           guests, amount, payment_status, source, wa_status, comment, created_at
