...
export async function getPublicMenu(lang = "ru") {
  ...
  const subs = await sql`
    SELECT id, category_id, sort, featured, title_ru, title_ka, title_en
  ...
  const items = await sql`
    SELECT id, category_id, subcategory_id, sort, title_ru, title_ka, title_en,
  ...
  return cats.map((c) => {
    ...
    return {
      id: c.id,
      title: pick(c, "title"),
      subcategories: (subsByCat.get(c.id) || []).filter((sc) => used.has(sc.id)),
      items: list
    };
  });
}
...
export async function updateMenuSubcategory(id, f) {
  ...
  const rows = await sql`
    UPDATE menu_subcategories SET
      ...
      title_ka = COALESCE(${f.title_ka ?? null}, title_ka),
      title_en = COALESCE(${f.title_en ?? null}, title_en),
      featured = COALESCE(${typeof f.featured === "boolean" ? f.featured : null}, featured)
    WHERE id = ${id}
    RETURNING *
  `;
  return rows[0] || null;
