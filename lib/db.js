...
export async function listUpcomingSessions() {
  ...
  return sql`
    SELECT s.id AS session_id, to_char(s.date, 'DD-MM-YYYY') AS date, s.time,
           e.title, e.price, e.poster_url, e.format
    FROM sessions s
    JOIN events e ON e.id = s.event_id
...
export async function getPublicMenu(lang = "ru") {
  ...
  const cats = await sql`
    SELECT id, sort, title_ru, title_ka, title_en
    FROM menu_categories WHERE visible = true ORDER BY sort ASC, created_at ASC
  `;
  if (!cats.length) return [];

  const items = await sql`
    SELECT id, category_id, sort, title_ru, title_ka, title_en,
           desc_ru, desc_ka, desc_en, price, photo_url
    FROM menu_items
    WHERE available = true AND category_id = ANY(${cats.map((c) => c.id)})
...
export async function createMenuItem(f) {
  ...
  const rows = await sql`
    INSERT INTO menu_items
      (id, category_id, sort, title_ru, title_ka, title_en,
       desc_ru, desc_ka, desc_en, price, photo_url)
    VALUES
      (${menuId("mi")}, ${f.category_id}, ${Number(f.sort) || 0}, ${f.title_ru},
...
export async function reorderMenu({ categories = [], items = [] }) {
  const sql = db();
