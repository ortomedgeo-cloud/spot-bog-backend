...
export async function sendEmailNotification(text, { subject = "SPOT — уведомление", apiKey, to, from } = {}) {
  apiKey = apiKey || required("RESEND_API_KEY");
  const toList = (to || required("NOTIFY_EMAIL_TO"))
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  from = from || process.env.NOTIFY_EMAIL_FROM || "onboarding@resend.dev";
