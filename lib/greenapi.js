...
// Отправка конкретному человеку по номеру телефона (в отличие от
// sendWhatsappNotification, которая шлёт в фиксированный чат персонала).
export async function sendWhatsappToPhone(phone, message) {
  const chatId = phoneToChatId(phone);
  if (!chatId) {
    const err = new Error(`Не похоже на номер телефона: ${phone}`);
    err.code = "NOT_A_PHONE";
    throw err;
  }
  return sendWhatsappNotification(message, { chatId });
}
