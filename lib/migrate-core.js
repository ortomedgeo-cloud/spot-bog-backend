...
// Normalizes a date cell to 'DD-MM-YYYY'. Handles:
//   - Excel/Sheets serial numbers (read via SERIAL_NUMBER — locale-proof, best)
//   - already 'DD-MM-YYYY'
//   - ISO '2026-07-14' / '2026-07-14T..'
//   - slash 'M/D/YYYY' (US locale, how live Sheets FORMATTED_STRING gives them)
export function toDdMmYyyy(value) {
  if (value == null || value === "") return "";

  // Serial number (days since 1899-12-30). Google returns dates this way with
  ...
  // slash M/D/YYYY (US) — live Sheets default. Month first.
...
export function toHhMm(value) {
  ...
  // Fractional serial (time-of-day). e.g. 0.875 => 21:00.
  if (typeof value === "number" || /^\d*\.\d+$/.test(String(value).trim())) {
    const frac = Number(value) % 1;
    if (frac > 0 || Number(value) < 1) {
