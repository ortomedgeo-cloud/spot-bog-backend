...
// нужные значения (isChannelConfigured) И он включён в таблице notify_settings
...
import { sendWhatsappNotification, phoneToChatId } from "./greenapi.js";
...
export const CHANNELS = {
  ...
  // Гостевой канал: те же ключи GreenAPI, но получатель — номер из брони, а не
  // фиксированный чат персонала. Поэтому GREEN_API_CHAT_ID ему не нужен, и
  // включается он отдельно: писать персоналу и писать гостю — разные решения.
  // В веерную рассылку notify() не попадает (broadcast:false) — только адресно,
  // через notifyGuest().
  guest_whatsapp: {
    title: "WhatsApp гостю",
    env: ["GREEN_API_ID_INSTANCE", "GREEN_API_TOKEN"],
    broadcast: false,
    send: (text, opts, vals) => {
      const chatId = phoneToChatId(opts && opts.phone);
      if (!chatId) {
        const err = new Error(`Не похоже на номер телефона: ${opts && opts.phone}`);
        err.code = "NOT_A_PHONE";
        throw err;
      }
...
// For callers outside notify()'s own broadcast loop (the Telegram webhook,
// which needs the bot token to answer button taps) — same env-then-DB
// resolution as everywhere else, just exposed for one channel at a time.
export async function getChannelValues(channel) {
  let secretsMap = {};
  try {
    secretsMap = await loadSecretsMap();
  } catch (e) {
    console.error("getChannelValues: не удалось прочитать notify_secrets", e);
  }
  return resolveValues(channel, secretsMap);
}
...
export async function notify(text, options = {}) {
  ...
  const candidates = only ? [only] : Object.keys(CHANNELS).filter((k) => CHANNELS[k].broadcast !== false);
...
// УВЕДОМЛЕНИЯ ГОСТЮ
// ---------------------------------------------------------------------------
// Пишет конкретному человеку на номер из брони. Контакт в брони — не всегда
// телефон: гости часто оставляют инстаграм. Это не ошибка, просто писать
// некуда, поэтому возвращаем not_a_phone и никого не тревожим.

export const GUEST_CHANNEL = "guest_whatsapp";

export async function notifyGuest(phone, text) {
  if (!phoneToChatId(phone)) {
    return { sent: false, reason: "not_a_phone" };
  }
  const res = await notify(text, { only: GUEST_CHANNEL, phone });
  const one = res.results && res.results[0];
  if (res.sent) return { sent: true };
  return { sent: false, reason: one ? "send_failed" : "not_active", error: one && one.error };
}
