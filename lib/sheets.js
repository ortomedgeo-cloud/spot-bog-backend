...
export async function getEventByCode(eventCode) {
  ...
  const row = data.find((r) => String(cellByHeader(r, idx, "eid") || "").trim() === target);
  if (!row) return null;
...
function buildBookingRow(header, booking) {
  const parsedDate = parseDdMmYyyy(booking.reserveDate);
  const parsedTime = parseTimeHm(booking.reserveTime);
  ...
  return buildRowFromHeader(header, valuesByName);
}
...
export async function appendBookingRowIfNotExists({
  ...
  const rows = await readRows(sheetName, "A:AE");
  if (!rows.length) throw new Error(`Sheet ${sheetName} is empty`);

  const [header, ...data] = rows;
  const idx = headerIndexMap(header);
  const bookingIdTarget = String(booking_id || "").trim();
  if (!bookingIdTarget) throw new Error("Missing booking_id for Bookings append");
  ...
  if (!event) throw new Error(`Event not found while writing booking: ${event_code}`);
