...
async function appendRow(sheetName, rowValues, rangeWidth) {
  ...
  await sheets.spreadsheets.values.append({
    spreadsheetId: required("GOOGLE_SHEET_ID"),
    // Pin to the header row (row 1), not an open-ended "A:P" column range.
    // An unbounded range lets Sheets' table auto-detection anchor on the
    // wrong starting column if any row in between is sparse/irregular —
    // that's what caused column A data to land in column M on the Bookings
    // sheet. Anchoring at A1 forces it to use the real header as the table.
